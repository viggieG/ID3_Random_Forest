...
def ID3(examples, default=None):
    ...
    if not attributes or all(entry[attribute] == '?' for attribute in attributes for entry in examples):
        node.label = most_common_class(examples)
        return node

    entropy_dataset = entropy(examples)
    gains = {attribute: information_gain(examples, attribute, entropy_dataset) for attribute in attributes if
             attribute != "Class"}
    ...
    for value in set(entry[max_attribute] for entry in examples if entry[max_attribute] != '?'):
        subset = [entry for entry in examples if entry[max_attribute] == value or entry[max_attribute] == '?']
        if subset != examples:  # Ensure that we're always reducing the set size
            node.children[value] = ID3(subset, default=most_common_class(subset))
        else:
            node.label = most_common_class(subset)
            return node
    return node
