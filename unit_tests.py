...
def testPruningOnHouseDataForQuestionFour(inFile):
    training_sizes = list(range(10, 301, 1))
    ...
    for train_size in training_sizes:
        ...
        for i in range(100):  # 100 repetitions
            ...
            # Adjust data split based on train_size
            train = data[:train_size]
            remaining = data[train_size:]
            valid = remaining[:len(remaining)//2]
            test = remaining[len(remaining)//2:]

            tree = ID3.ID3(train, 'democrat')
            acc = ID3.test(tree, test)
            tree = ID3.ID3(train + valid, 'democrat')  # Train on combined train + valid for unpruned
            withoutPruning.append(ID3.test(tree, test))

            ID3.prune(tree, valid)  # Pruning based on validation set
            withPruning.append(ID3.test(tree, test))

        average_with_pruning.append(sum(withPruning) / len(withPruning))
        average_without_pruning.append(sum(withoutPruning) / len(withoutPruning))

    # Plotting the results
    plt.plot(training_sizes, average_with_pruning, label="With Pruning", marker='o')
    plt.plot(training_sizes, average_without_pruning, label="Without Pruning", marker='o')
...
if __name__ == '__main__':
    ...
    #testPruningOnHouseDataForQuestionFour('house_votes_84.data')
    testPruningOnCandyData("candy.data")
